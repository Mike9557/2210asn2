public class TestDict {
  ...
  /*
  ** Test program for the TTTDictionary class.
  */

  public static void main(String[] args) {
    TTTDictionary dict = new TTTDictionary(9887);
    TTTRecord pair;
    ...
    if (test[1] || test[3] || test[6])
       try {
         dict.put(new TTTRecord("answer", 42,1));
    ...
    if (test[2])
       try {
         int x = dict.put(new TTTRecord("answer", 56,1));
    ...
    if (test[7])
	   ...
	   for (i = 0; i < 10000; ++i) {
	       ...
	       for (j = 0; j < 5; ++j) s += s;
	       collisions += dict.put(new TTTRecord(s,i,i));
	       
	   }
         System.out.println("   Test 7 succeeded");
         System.out.println(dict.numElements());
       } catch (Exception e) {
    ...
    if (test[9])
	   ...
	   System.out.println("***Test 9 failed123");
